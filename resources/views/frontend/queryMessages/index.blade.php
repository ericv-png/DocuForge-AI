        ...
        <div class="col-md-12">
            @can('query_message_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.query-messages.create') }}">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-QueryMessage">
                            <thead>
                                <tr>
                                    <th>
                                    ...
                                    <tr data-entry-id="{{ $queryMessage->id }}">
                                        ...
                                        <td>
                                            @can('query_message_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.query-messages.show', $queryMessage->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('query_message_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.query-messages.edit', $queryMessage->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('query_message_delete')
                                                <form action="{{ route('frontend.query-messages.destroy', $queryMessage->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('query_message_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.query-messages.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-QueryMessage:not(.ajaxTable)').DataTable({ buttons: dtButtons })
