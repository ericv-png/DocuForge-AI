        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    {{ trans('global.create') }} {{ trans('cruds.extractedData.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.extracted-datas.store") }}" enctype="multipart/form-data">
                        @method('POST')
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="source_file_id" id="source_file_id" required>
                                @foreach($source_files as $id => $entry)
                                    <option value="{{ $id }}" {{ old('source_file_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="extracted_data" id="extracted_data" required>{{ old('extracted_data') }}</textarea>
