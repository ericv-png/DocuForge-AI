...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.query-messages.update", [$queryMessage->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="reference">{{ trans('cruds.queryMessage.fields.reference') }}</label>
                <input class="form-control {{ $errors->has('reference') ? 'is-invalid' : '' }}" type="text" name="reference" id="reference" value="{{ old('reference', $queryMessage->reference) }}" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('message') ? 'is-invalid' : '' }}" name="message" id="message">{{ old('message', $queryMessage->message) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tokens') ? 'is-invalid' : '' }}" type="number" name="tokens" id="tokens" value="{{ old('tokens', $queryMessage->tokens) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ai_model') ? 'is-invalid' : '' }}" type="text" name="ai_model" id="ai_model" value="{{ old('ai_model', $queryMessage->ai_model) }}">
