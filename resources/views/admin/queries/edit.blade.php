    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.queries.update", [$query->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('reference') ? 'is-invalid' : '' }}" type="text" name="reference" id="reference" value="{{ old('reference', $query->reference) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $query->name) }}" required>
                @if($errors->has('name'))
                    <div class="invalid-feedback">
                        {{ $errors->first('name') }}
                    </div>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('encrypted') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="encrypted" id="encrypted" value="1" {{ $query->encrypted || old('encrypted', 0) === 1 ? 'checked' : '' }}>
