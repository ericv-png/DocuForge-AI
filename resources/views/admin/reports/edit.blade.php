    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.reports.update", [$report->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('reference') ? 'is-invalid' : '' }}" type="text" name="reference" id="reference" value="{{ old('reference', $report->reference) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $report->name) }}">
                @if($errors->has('name'))
                    <div class="invalid-feedback">
                        {{ $errors->first('name') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $report->description) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('query') ? 'is-invalid' : '' }}" name="query_id" id="query_id">
                    @foreach($queries as $id => $entry)
                        <option value="{{ $id }}" {{ (old('query_id') ? old('query_id') : $report->query->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('query'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('query_message') ? 'is-invalid' : '' }}" name="query_message_id" id="query_message_id">
                    @foreach($query_messages as $id => $entry)
                        <option value="{{ $id }}" {{ (old('query_message_id') ? old('query_message_id') : $report->query_message->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <div class="needsclick dropzone {{ $errors->has('generated_file') ? 'is-invalid' : '' }}" id="generated_file-dropzone">
                </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tokens') ? 'is-invalid' : '' }}" type="number" name="tokens" id="tokens" value="{{ old('tokens', $report->tokens) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ai_model') ? 'is-invalid' : '' }}" type="text" name="ai_model" id="ai_model" value="{{ old('ai_model', $report->ai_model) }}">
