@can('data_source_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.data-sources.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.dataSource.title_singular') }}
            </a>
        </div>
    </div>
@endcan

<div class="card">
    <div class="card-header">
        {{ trans('cruds.dataSource.title_singular') }} {{ trans('global.list') }}
    </div>

    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-categoryDataSources">
                <thead>
                    <tr>
                        <th width="10">

                        </th>
                        <th>
                            {{ trans('cruds.dataSource.fields.id') }}
                        </th>
                        <th>
                            {{ trans('cruds.dataSource.fields.reference') }}
                        </th>
                        <th>
                            {{ trans('cruds.dataSource.fields.name') }}
                        </th>
                        <th>
                            {{ trans('cruds.dataSource.fields.category') }}
                        </th>
                        <th>
                            {{ trans('cruds.dataSource.fields.type') }}
                        </th>
                        <th>
                            {{ trans('cruds.dataSource.fields.url') }}
                        </th>
                        <th>
                            {{ trans('cruds.dataSource.fields.file') }}
                        </th>
                        <th>
                            {{ trans('cruds.dataSource.fields.status') }}
                        </th>
                        <th>
                            &nbsp;
                        </th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($dataSources as $key => $dataSource)
                        <tr data-entry-id="{{ $dataSource->id }}">
                            <td>

                            </td>
                            <td>
                                {{ $dataSource->id ?? '' }}
                            </td>
                            <td>
                                {{ $dataSource->reference ?? '' }}
                            </td>
                            <td>
                                {{ $dataSource->name ?? '' }}
                            </td>
                            <td>
                                @foreach($dataSource->categories as $key => $item)
                                    <span class="badge badge-info">{{ $item->name }}</span>
                                @endforeach
                            </td>
                            <td>
                                {{ App\Models\DataSource::TYPE_SELECT[$dataSource->type] ?? '' }}
                            </td>
                            <td>
                                {{ $dataSource->url ?? '' }}
                            </td>
                            <td>
                                @if($dataSource->file)
                                    <a href="{{ $dataSource->file->getUrl() }}" target="_blank">
                                        {{ trans('global.view_file') }}
                                    </a>
                                @endif
                            </td>
                            <td>
                                {{ App\Models\DataSource::STATUS_SELECT[$dataSource->status] ?? '' }}
                            </td>
                            <td>
                                @can('data_source_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.data-sources.show', $dataSource->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('data_source_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.data-sources.edit', $dataSource->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('data_source_delete')
                                    <form action="{{ route('admin.data-sources.destroy', $dataSource->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
                                        <input type="hidden" name="_method" value="DELETE">
                                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                        <input type="submit" class="btn btn-xs btn-danger" value="{{ trans('global.delete') }}">
                                    </form>
                                @endcan

                            </td>

                        </tr>
                    @endforeach
                </tbody>
            </table>
        </div>
    </div>
</div>

@section('scripts')
@parent
<script>
    $(function () {
  let dtButtons = $.extend(true, [], $.fn.dataTable.defaults.buttons)
@can('data_source_delete')
  let deleteButtonTrans = '{{ trans('global.datatables.delete') }}'
  let deleteButton = {
    text: deleteButtonTrans,
    url: "{{ route('admin.data-sources.massDestroy') }}",
    className: 'btn-danger',
    action: function (e, dt, node, config) {
      var ids = $.map(dt.rows({ selected: true }).nodes(), function (entry) {
          return $(entry).data('entry-id')
      });

      if (ids.length === 0) {
        alert('{{ trans('global.datatables.zero_selected') }}')

        return
      }

      if (confirm('{{ trans('global.areYouSure') }}')) {
        $.ajax({
          headers: {'x-csrf-token': _token},
          method: 'POST',
          url: config.url,
          data: { ids: ids, _method: 'DELETE' }})
          .done(function () { location.reload() })
      }
    }
  }
  dtButtons.push(deleteButton)
@endcan

  $.extend(true, $.fn.dataTable.defaults, {
    orderCellsTop: true,
    order: [[ 1, 'desc' ]],
    pageLength: 100,
  });
  let table = $('.datatable-categoryDataSources:not(.ajaxTable)').DataTable({ buttons: dtButtons })
  $('a[data-toggle="tab"]').on('shown.bs.tab click', function(e){
      $($.fn.dataTable.tables(true)).DataTable()
          .columns.adjust();
  });
  
})

</script>
@endsection